...
### 21:19 — `[Add] TrenchBroom (éditeur de niveaux Quake) via AppImage GitHub` (commit `c9cdd87`)

- **Contexte** : la piste `.deb` de `doomcker/Dockerfile.bak` (v2022.1, releases GitHub) fonctionne toujours mais n'est plus maintenue par upstream — TrenchBroom ne publie plus de `.deb` depuis, uniquement une AppImage (`v2026.2` au moment du test).
- **Vérification de la piste `.deb`** : l'URL v2022.1 répond toujours et `apt-get install -y <chemin.deb>` (résolution auto des dépendances Qt5) réussit — mais un `dpkg -i` brut sans résolution de dépendances (comme dans `Dockerfile.bak`) aurait cassé le build.
- **Décision** : utilisation de la dernière release (`v2026.2`, AppImage) plutôt que le vieux `.deb`. L'AppImage est extraite au build (`--appimage-extract`) plutôt que lancée directement, car aucun `/dev/fuse` n'est monté dans le conteneur au runtime (`Makefile` ne le prévoit pas) — FUSE est nécessaire pour monter une AppImage telle quelle. Contenu extrait déplacé vers `/opt/trenchbroom`.
- **Piège découvert** : un symlink direct `/usr/games/trenchbroom -> /opt/trenchbroom/AppRun` casse le script `AppRun` (généré par `linuxdeploy`), qui résout son propre répertoire via `dirname "$0"` sans suivre les liens symboliques — `$0` restait `/usr/games/trenchbroom`, donc il cherchait ses fichiers annexes (`apprun-hooks/`) dans `/usr/games/` au lieu de `/opt/trenchbroom/`. Corrigé avec un script wrapper (`exec /opt/trenchbroom/AppRun "$@"`) à la place du symlink.
- **Validation** : build réel du stage `system`, échec du symlink détecté par lancement réel sous Xvfb (`Xvfb :99` + `DISPLAY=:99`), correction, nouveau build, build complet 3 stages, arrêt/relance du conteneur, lancement réel sur le display `:1` du conteneur (celui utilisé par x11vnc/noVNC en production) : l'application démarre sans erreur (reste active jusqu'au timeout de test, comportement normal d'une appli GUI en attente).
